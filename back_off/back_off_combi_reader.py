...
def _raw_data(data_path=None):
  train_path = os.path.join(data_path, "model_300_unk.train.nltk_tokenizer.txt.50class.npy")
  valid_path = os.path.join(data_path, "model_300_unk.valid.nltk_tokenizer.txt.50class.npy")
  test_path = os.path.join(data_path, "model_300_unk.test.nltk_tokenizer.txt.50class.npy")
  ...
  test_path = os.path.join(data_path, "test.nltk_tokenizer.txt.mono")
...
def sample_producer(raw_data, raw_class, batch_size, num_steps, name=None):
  with tf.name_scope(name, "SampleProducer", [raw_data, raw_class, batch_size, num_steps]):
    ...
    # class at time t
    class_t = tf.strided_slice(data_class, [0, i * num_steps],
                         [batch_size, (i + 1) * num_steps])
    class_t.set_shape([batch_size, num_steps])
    # class at time t+1
    class_tplus1 = tf.strided_slice(data_class, [0, i * num_steps + 1],
                         [batch_size, (i + 1) * num_steps + 1])
    class_tplus1.set_shape([batch_size, num_steps])
    return x, y, class_t, class_tplus1
