...
# -*- coding: utf-8 -*-
"""
python back_off_LM.py\
 --gpus=3\
  --inference=False\
   --save_path="tmp/backoff_with_class_t_plus_1"\
    --n_clusters="100"
...
import back_off_reader as CSReader
import logging
...
class InputData(object):
  ...
  def __init__(self, config, data, data_class, name=None):
    ...
    self.input_data, self.targets, self.class_t1 = CSReader.sample_producer(data, data_class, batch_size, num_steps, name=name)
    ...
    def __init__(self, is_training, config, input_):    
        ...
        with tf.device("/cpu:0"):
            ...
            with tf.variable_scope("Embedding"):
                ...
                # shape [batch_size, num_steps, hidden_dim]
                cluster_inputs = tf.nn.embedding_lookup(cluster_embedding, input_.class_t1)
                inputs = tf.concat([embedding_inputs, cluster_inputs], 2)
        ...
        with tf.device('/device:GPU:0'):
            ...
            # with tf.variable_scope("CLASS_RNN") as class_path1:
            ...
            #     cluster_weights = tf.reshape(tf.concat(outputs, 1), shape=(-1, config.hidden_size))
            ...
            if not is_training:
                return
            # with tf.variable_scope("CLASS_softmax") as class_path2:
            #
            #     softmax_w_class = tf.get_variable(
            #         "softmax_w_class",
            #         [config.hidden_size, config.class_vocab],
            #         dtype=tf.float32)
            #
            ...
            #         dtype=tf.float32)
            #
            #     logits = tf.nn.xw_plus_b(word_weights, softmax_w_class, softmax_b_class)
            #     logits = tf.reshape(logits, [config.batch_size, config.num_steps, config.class_vocab])
            #     loss_class = tf.contrib.seq2seq.sequence_loss(
            #         logits,
            #         input_.targets_class,
            #         tf.ones([self.batch_size, self.num_steps], dtype=tf.float32),
            #         average_across_timesteps=False,
            #         average_across_batch=True)
...
def main(_):
    ...
    with tf.Graph().as_default() as g:
        ...
        with tf.Session() as sess:
            ...
            if is_inference:
                saver.restore(sess, "tmp/backoff_with_class_t_plus_1/model.ckpt-4473")
