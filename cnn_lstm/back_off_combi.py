	...
	def __init__(self, is_training, config, input_):
		...
		with tf.variable_scope("Class_Net"):
			with tf.device("/cpu:0"):
				...
				class_embedding = np.load("data/model_300_unk_embedding.npy_50c_embed.npy")
				# # do normalization
				...
				print("the input shape is ", cluster_inputs.get_shape())
				class_target = tf.nn.embedding_lookup(cluster_embedding, input_.class_t1)
				print("the target shape is ", class_target.get_shape())
			# add in convolutional layer to learn the average word vector and store in kernel
			# kernel size is 100 implying 100 base words
			cluster_inputs = tf.reshape(cluster_inputs, [config.batch_size, config.num_steps, size, 1])
			print("cluster input reshape to ", cluster_inputs.get_shape())
			conv1 = tf.layers.conv2d(
				inputs=cluster_inputs,
				filters=config.kernel_filter,
				kernel_size=[1, size],
				padding="valid",
				use_bias=False,
				activation=tf.nn.relu,
				name="conv2d")
			# the ouptut of conv is of shape(batch, time_step, kernel_filter)
			print("conv1 output shape is ", conv1.get_shape())
			conv1 = tf.reshape(conv1, [config.batch_size, config.num_steps, config.kernel_filter])
			print("conv1 reshaped to ", conv1.get_shape())
			def make_cell_class():
				cell = tf.contrib.rnn.LSTMBlockCell(
					config.kernel_filter, forget_bias=0.0)
			...
			# initial state for each batch is zero
			self._initial_state_cluster = cluster_cell.zero_state(config.batch_size, tf.float32)
			state_class = self._initial_state_cluster
			# unstack input according to time steps
			cluster_inputs_unstacked = tf.unstack(conv1, num=config.num_steps, axis=1)
			...
			# the output class contains the 100 kernel weights
			cluster_output = tf.reshape(tf.concat(outputs_class, 1), shape=(-1, config.kernel_filter))
			print("the cluster_output shape is ", cluster_output.get_shape())
			# softmax will be replaced by the kernel reconstruction method
			# first get kerenl
			word_kernel = tf.get_default_graph().get_tensor_by_name('Model/Class_Net/conv2d/kernel:0')
			word_kernel = tf.reshape(word_kernel, [size, config.kernel_filter])
			# this is the predicted
			softmax_w_class = tf.get_variable(
				"kernel_weights",
				[config.kernel_filter, config.hidden_size],
				trainable=False)
			softmax_w_class.assign_add(tf.transpose(word_kernel))

			# logits_class = tf.matmul(cluster_output, word_kernel, transpose_b=True)
			logits_class = tf.matmul(cluster_output, softmax_w_class)
			# softmax_w_class = tf.get_variable(
			# 	"softmax_w_class",
			# 	[config.kernel_filter, config.hidden_size],
			# 	dtype=tf.float32,
			...
			logits_class = tf.reshape(logits_class, [config.batch_size, config.num_steps, size])
			loss_class = tf.losses.absolute_difference(
			...
			self._cost_class = tf.reduce_mean(loss_class)
		...
		with tf.variable_scope("Word_Net"):
			...
			with tf.control_dependencies([embedding_inputs, logits_class]):
				...
				inputs = tf.concat([embedding_inputs, logits_class], 2)
	...
	for step in range(model.input.epoch_size):
		...
		if verbose and step % (model.input.epoch_size // 10) == 1:
			logging.info("%.3f class ppl: %.3f perplexity: %.3f speed: %.0f wps" %
			             (step * 1.0 / model.input.epoch_size, costs_class / iters, np.exp(costs / iters),
