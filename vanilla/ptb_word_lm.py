...
# LOOK HERE
# python ptb_word_lm.py --data_path="data" --save_path="temp/ptbmodel.ckpt" --num_gpus=2 --model="medium"

# <editor-fold desc="logging, FLAGS and CUDA setup">
# get TF logger
...
fh = logging.FileHandler('log/ptbmodel.log')
# print("set log path")
...
class PTBModel(object):
	...
	def __init__(self, is_training, config, input_):
		...
		with tf.device("/cpu:0"):
			...
			# instead of one-hot, use ids directly for embedding lookup
		...
		if not is_training:
			return
		# from here until the end is mystery. need to refer to the original documentation
	...
	# offers different type of networks for training
...
def decode_text(session, model, iters=15, id_to_word=None):
	# decoding
	start_token = [[0]]
	state = session.run(model.initial_state)
	...
	for i in range(iters):
		...
		# update feed
		state = vals["final_state"]
		print(id_to_word[softmax_argmax])
...
def main(_):
	...
	# for evaluation reset batch_size and num_step
